...
def process(videofile):
    ...
    start = time.time()
    videofile = os.path.join('media/video', videofile)
    ...
    while True:
        ...
        for ((top, right, bottom, left), name) in zip(boxes, names):
            ...
            cv2.putText(frame, name, (left, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 2)

            if writer is None:
                fourcc = cv2.VideoWriter_fourcc(*"MJPG")
                writer = cv2.VideoWriter("media/video/response.avi", fourcc, 20, (frame.shape[1], frame.shape[0]), True)
